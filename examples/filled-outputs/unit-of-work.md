...
- Follow `core/units-generation.md`.
- Sizing criteria: follow `core/unit-sizing.md`.
...
| UOW-2 | Repurchase determination query | S | UOW-1 | TODO |
...
- Dependencies: None
- Size: S
- Acceptance criteria: Entity and Repository exist and basic CRUD tests pass
- Verification method: unit tests

## UOW-2. Repurchase determination query
- Responsibility: query the list of customer IDs with a completed payment history within the last N days
- Expected location: domains/domain-rds/src/.../order/repository/
- Dependencies: UOW-1
- Size: S
- Acceptance criteria: accurately returns the list of repurchase-target customers based on the period condition
- Verification method: unit tests (based on test data)

## UOW-3. Coupon auto-issuance batch
- Responsibility: query repurchase targets via scheduler → issue coupons to customers who have not been issued one → record issuance history
- Expected location: center/back-end/src/.../campaign/batch/
- Dependencies: UOW-1, UOW-2
- Size: M
- Acceptance criteria: when the batch runs, coupons are issued to target customers, no duplicate issuance, completes within 5 minutes for 100,000 records
...
- Dependencies: UOW-1
- Size: S
- Acceptance criteria: campaign create/modify/query/ON/OFF works via API
- Verification method: integration tests

## UOW-5. Coupon usage validation and payment integration
- Responsibility: coupon application request at order time → validity check → apply discount → mark as used
- Expected location: center/back-end/src/.../payment/service/, .../coupon/service/
- Dependencies: UOW-1
- Size: M
- Acceptance criteria: discount is reflected when a valid coupon is applied, expired/already-used coupons are blocked, orders below the minimum amount are blocked
- Verification method: integration tests
...
2. UOW-2 — a prerequisite for the batch (UOW-3)
3. UOW-4 — the batch is only meaningful once the administrator has created a campaign first
4. UOW-3 — the actual issuance operation
5. UOW-5 — the flow that uses the issued coupons
6. UOW-6 — status queries are only meaningful once data has accumulated

## Sizing criteria
Follow `core/unit-sizing.md`.
